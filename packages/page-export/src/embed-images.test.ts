...
import { embedImagesAndRewriteLinks } from './embed-images.ts'
...
describe('embedImagesAndRewriteLinks', () => {
  it('scopes the file lookup to the workspace (plus public files)', async () => {
    const storage = makeStorage({})
    const prisma = makePrisma([])
    await embedImagesAndRewriteLinks('<img src="/api/files/abc">', {
      storage,
      prisma,
      baseUrl,
      workspaceId,
    ...
    expect(prisma.file.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ OR: [{ workspaceId }, { isPublic: true }] }),
      }),
    )
  })
  ...
  it('replaces /api/files/<id> images with data: URIs', async () => {
    ...
    const out = await embedImagesAndRewriteLinks(html, { storage, prisma, baseUrl, workspaceId })
