...
export function checkInvariants(data: GenogramPageData): ValidationIssue[] {
  ...
  for (const u of Object.values(unions)) {
    ...
    if (!female) {
      ...
      issues.push(
        issue(['unions', u.id, 'femalePartnerId'], "female partner must have sex:'female'"),
      )
    }
  ...
  for (const bg of Object.values(birthGroups)) {
    if (bg.memberIds.length < 2) {
      issues.push(
        issue(['birthGroups', bg.id, 'memberIds'], 'birth group requires at least 2 members'),
      )
    }
    ...
    if (containing.size > 1) {
      issues.push(
        issue(['birthGroups', bg.id], 'birth group members must belong to a single child group'),
      )
    }
  }
  ...
  for (const pl of Object.values(pregnancyLosses)) {
    if (!childGroups[pl.childGroupId]) {
      issues.push(
        issue(['pregnancyLosses', pl.id, 'childGroupId'], `unknown child group ${pl.childGroupId}`),
      )
    }
  }
