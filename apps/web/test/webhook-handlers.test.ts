...
describe("handlePaymentSucceeded", () => {
  ...
  beforeEach(async () => {
    ...
    await prisma.subscription.deleteMany({ where: { user: { email: { contains: TEST_EMAIL_SUFFIX } } } })
    ...
    const user = await prisma.user.create({
      data: { email: `wh${TEST_EMAIL_SUFFIX}`, emailVerified: true, name: "WH", firstName: "WH", lastName: "Test" },
    })
    userId = user.id
  ...
  it("transitions Order to PAID and creates ACTIVE Subscription", async () => {
    ...
    await handlePaymentSucceeded(
      { yookassa: yk as any, prisma },
      { id: "pmt_wh_test_1" } as Payment,
    )
  ...
  it("is idempotent: second invocation does not duplicate Subscription", async () => {
    ...
    await handlePaymentSucceeded({ yookassa: yk as any, prisma }, { id: "pmt_wh_test_1" } as Payment)
    await handlePaymentSucceeded({ yookassa: yk as any, prisma }, { id: "pmt_wh_test_1" } as Payment)
