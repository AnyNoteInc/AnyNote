...
describe("IndexingProcessor", () => {
  ...
  it("processes normalized subchunks end-to-end when page is valid", async () => {
    ;(mockPrisma.page.findUnique as jest.Mock).mockResolvedValue({
      id: "p1",
      type: "TEXT",
      ownership: "TEXT",
    ...
    ;(mockProcessing.normalize as jest.Mock)
      .mockResolvedValueOnce(["a-1", "a-2"] as never)
      .mockResolvedValueOnce(["b-1"] as never)
    ;(mockEmbed.embed as jest.Mock)
      .mockResolvedValueOnce([0.1, 0.2] as never)
      .mockResolvedValueOnce([0.3, 0.4] as never)
      .mockResolvedValueOnce([0.5, 0.6] as never)
  ...
  it("drops empty normalized subchunks", async () => {
    ;(mockPrisma.page.findUnique as jest.Mock).mockResolvedValue({
      id: "p1",
      type: "TEXT",
      ownership: "TEXT",
    ...
    expect(mockQdrant.upsert).toHaveBeenCalledWith([
      expect.objectContaining({ vector: [0.5], payload: expect.objectContaining({ chunkIndex: 0 }) }),
    ])
  })
})
