...
from agents.apps.processing.schemas import EmbeddingValidationRequestSchema
...
async def test_validate_embedding_detects_vector_size() -> None:
    ...
    res = await uc(EmbeddingValidationRequestSchema(provider='ollama', modelSlug='nomic', connection={'baseUrl': 'http://o:1'}))
...
async def test_validate_embedding_failure_is_caught() -> None:
    ...
    uc = ValidateEmbeddingUseCase(embedding_factory=factory)
    res = await uc(EmbeddingValidationRequestSchema(provider='ollama', modelSlug='x', connection={}))
...
async def test_validate_embedding_rejects_empty_vector() -> None:
    ...
    factory.make.return_value = emb
    uc = ValidateEmbeddingUseCase(embedding_factory=factory)
    res = await uc(EmbeddingValidationRequestSchema(provider='ollama', modelSlug='m', connection={'baseUrl': 'http://o:1'}))
    assert res.ok is False
