...
Avoids langchain-mcp-adapters churn by speaking the JSON-RPC dialect
directly. We fetch ``tools/list`` once, wrap each entry in a LangChain
StructuredTool that invokes ``tools/call`` on demand. Errors are
surfaced as strings so the agent loop can recover instead of crashing.
"""
...
from agents.schemas.generate import McpServer
...
def _wrap_tool(server: McpServer, entry: dict[str, Any]) -> StructuredTool:
    ...
    server_snapshot = server  # closure capture
...
def _make_async_runner(coroutine: Callable[..., Any]) -> Callable[..., Any]:
    # StructuredTool.from_function expects a coroutine; pass through.
    return coroutine
