...
test('dblclick по картинке открывает сплит-панель; OpenInFull/CloseFullscreen переключают режимы', async ({
  ...
  // Крошечный 1×1 PNG рендерится ~1px шириной и полностью перекрыт оверлеями
  // ресайз-хендлов (width:8 у каждого края), поэтому нативный .dblclick() не
  // доходит до <img>. Диспатчим dblclick прямо на элемент — паттерн «клик под
  // виджет-оверлеем»; React ловит его делегированным слушателем на корне.
